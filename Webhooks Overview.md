...
4. Process webhook idempotently (dedup by `id`, or fallback key for events with `id = null`).
5. Return `2xx` as soon as webhook is accepted/processed.
...
| Retry policy | Up to 3 attempts on `RestClientException` (includes network/client HTTP exceptions, including `RestClientResponseException`) |
...
| Use webhook `id` as primary dedup key | Stable unique event identifier when present |
| Use fallback dedup key when `id = null` | Some events do not provide `id`; recommended key: `type + clientId + createdAt` |
...
| Field | Type | Meaning |
|---|---|---|
| `id` | `string \| null` | Webhook event ID; can be `null` for some additional events |
| `type` | `string` | Event type |
| `createdAt` | `string` | Event creation timestamp, format `yyyy-MM-dd'T'HH:mm:ss` |
